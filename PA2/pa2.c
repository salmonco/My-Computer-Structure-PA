...
static int process_instruction(unsigned int instr)
{
	...
	// 0x012a4020 ÀÌ·± instructionÀÌ ÀÖÀ¸¸é opcode(6 bits)¸¦ º»´Ù.
	// opcode°¡ 0ÀÌ¸é rÆ÷¸ËÀÌ°í funct(6 bits)¸¦ º¸°í ¸í·É¾î ÀÌ¸§À» °áÁ¤ÇÑ´Ù.
	// opcode°¡ 0ÀÌ ¾Æ´Ï¸é iÆ÷¸ËÀÌ°Å³ª jÆ÷¸ËÀÌ°í opcode·Î ¸í·É¾î ÀÌ¸§À» °áÁ¤ÇÑ´Ù.

	// ÇØ´ç Æ÷¸Ë¿¡ ¸Â°Ô instructionÀ» ÂÉ°³¼­ º¸°í ¿¬»êÀÌ ÇÊ¿äÇÏ¸é ¼öÇàÇÑ´Ù.
	// ·¹Áö½ºÅÍ ¹øÈ£°¡ registers ¹è¿­ÀÇ ÀÎµ¦½º ¹øÈ£¿Í °°À¸¹Ç·Î ¹è¿­¿¡ °ªÀ» °¡Á®¿À°Å³ª ³Ö¾îÁØ´Ù.

	// iÆ÷¸ËÀÇ lw, sw -> ¸Þ¸ð¸®¿¡¼­ °ª °¡Á®¿À°í ³Ö¾îÁà¾ß ÇÔ
	// rÆ÷¸ËÀÇ jr, iÆ÷¸ËÀÇ beq, bne, jÆ÷¸ËÀÇ j, jal -> pc ÀÌµ¿ÇØÁà¾ß ÇÔ

	// unsigned, signed -> unsigned·Î ´õÇÏ³ª signed·Î ´õÇÏ³ª °ªÀº ¶È°°ÀÌ ³ª¿È. ÄÄÇ»ÅÍ¿¡ ÀúÀåµÇ´Â ºñÆ®´Â °°±â ¶§¹®

	// ¹®Á¦ : sra, lw, sw, ra (ÇØ°á)
	// sra -> (signed)·Î Çüº¯È¯ / lw, sw -> 1 word¾¿ °¡Á®¿À°í ÀúÀåÇÏ´Â °Å¿´À½ / ra -> ÄÚµå ¼ø¼­ ¹Ù²Þ

	// ¹®Á¦: bneÇÒ ¶§ pc¿¡¼­ ¸¶ÀÌ³Ê½º °ª °è»êÀÌ ¾ÈµÊ -> immedi °ªÀ» ºñÆ® ¼ö¿¡ ¸Â°Ô signedÀ¸·Î Çüº¯È¯Çß´õ´Ï ÇØ°áµÊ

	// ¹®Á¦: ±×³É basic¿¡¼­´Â signed °í·Á ¾ÈÇÏ´Â °Í °°°í, run basicÇÏ¸é signed·Î °í·ÁµÇ´Â µí. ¿Ö???
	...
	if (opcode == 0) { // R-format
		...
		case 0x02: // srl
			...
			break;
		case 0x03: // sra
			registers[rd] = (signed)registers[rt] >> shamt; // signed·Î shift
			break;
		case 0x2a: // slt
			...
			break;
		case 0x08: // jr
			pc = registers[rs]; // rs ·¹Áö½ºÅÍ°¡ °¡Áö°í ÀÖ´Â ÁÖ¼ÒÀ§Ä¡·Î jump
		}
	}
		...
		case 0x02: // j
			...
			break;
		case 0x03: // jal
			registers[31] = pc; // ra°¡ jal ´ÙÀ½ÀÇ instructionÀ» °¡¸®Å°µµ·Ï ÇÑ´Ù.
			pc = (pc >> 27 << 27) | (immedi << 2); // pc[31...28](4 bits) + immedi(26 bits) + 00(2 bits)
		}
	}
	else { // I-format
		...
		case 0x0d: // ori
			registers[rt] = registers[rs] | (unsigned short)immedi;
			break;
		case 0x23: // lw -> 1 word (32 bits)¾¿ °¡Á®¿Â´Ù.
			...
			break;
		case 0x2b: // sw -> 1 word (32 bits)¾¿ ÀúÀåÇÑ´Ù.
		...
		case 0x0a: // slti
			...
			break;
		case 0x04: // beq
			if (registers[rt] == registers[rs]) pc = pc + 4 * immedi; // ¿©±â¼­ immedi´Â offsetÀÓ
			break;
		case 0x05: // bne
			if (registers[rt] != registers[rs]) pc = pc + 4 * immedi; // ¿©±â¼­ immedi´Â offsetÀÓ
		}
...
{
	// ¸Þ¸ð¸®¿¡ instructionÀ» ³Ö¾îÁà¾ß ÇÔ. ¹è¿­ ÇÑ Ä­¿¡ 8 ºñÆ®¾¿ -> memory[] = 0x00
	// fgets·Î ÆÄÀÏ ¾È¿¡ µ¥ÀÌÅÍ¸¦ ÇÑ ÁÙ¾¿ ÀÐ¾î¼­ ¸Þ¸ð¸®¿¡ ·ÎµùÇÑ´Ù.
...
{
	// ¸Þ¸ð¸®¿¡ ·ÎµåµÈ instructionÀ» process_instruction(instr)·Î ½ÇÇà½ÃÄÑÁà¾ß ÇÔ
	pc = INITIAL_PC;
	unsigned int instr;
